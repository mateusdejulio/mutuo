...
class CheckoutPremiumUsuario extends StatefulWidget {
  final String cpf;
  final String nomeInicial;

  const CheckoutPremiumUsuario({
    super.key,
    required this.cpf,
    this.nomeInicial = '',
  });
  ...
  @override
  State<CheckoutPremiumUsuario> createState() =>
      _CheckoutPremiumUsuarioState();
}

class _CheckoutPremiumUsuarioState extends State<CheckoutPremiumUsuario> {
  ...
  BandeiraCartaoUsuario _bandeira = BandeiraCartaoUsuario.desconhecida;
  ...
  BandeiraCartaoUsuario _detectarBandeira(String numeroComMascara) {
    final digitos = numeroComMascara.replaceAll(RegExp(r'[^0-9]'), '');
    if (digitos.isEmpty) return BandeiraCartaoUsuario.desconhecida;

    if (RegExp(r'^4').hasMatch(digitos)) {
      return BandeiraCartaoUsuario.visa;
    }
    if (RegExp(
      r'^(5[1-5]|2(2[2-9][1-9]|[3-6][0-9]{2}|7[0-1][0-9]|720))',
    ).hasMatch(digitos)) {
      return BandeiraCartaoUsuario.mastercard;
    }
    if (RegExp(r'^3[47]').hasMatch(digitos)) {
      return BandeiraCartaoUsuario.amex;
    }
    if (RegExp(r'^(606282|3841)').hasMatch(digitos)) {
      return BandeiraCartaoUsuario.hipercard;
    }
    if (RegExp(
      ...
      return BandeiraCartaoUsuario.elo;
    }
    return BandeiraCartaoUsuario.desconhecida;
  }

  String? get _bandeiraAsset {
    switch (_bandeira) {
      case BandeiraCartaoUsuario.visa:
        return 'assets/bandeiras/visa.png';
      case BandeiraCartaoUsuario.mastercard:
        return 'assets/bandeiras/mastercard.png';
      case BandeiraCartaoUsuario.elo:
        return 'assets/bandeiras/elo.png';
      case BandeiraCartaoUsuario.amex:
        return 'assets/bandeiras/amex.png';
      case BandeiraCartaoUsuario.hipercard:
        return 'assets/bandeiras/hipercard.svg';
      case BandeiraCartaoUsuario.desconhecida:
        return null;
    }
  }
  ...
  Future<void> _confirmarAssinatura() async {
    ...
    final resultado = await _api.ativarPremiumUsuario(widget.cpf);
    if (!mounted) return;
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    _campoInput(
                      controller: _cvvCtrl,
                      hint: _bandeira == BandeiraCartaoUsuario.amex
                          ? '1234'
                          : '123',
                      tipoNumerico: true,
  ...
  Widget _iconeBandeira(BandeiraCartaoUsuario bandeira, {double tamanho = 32}) {
