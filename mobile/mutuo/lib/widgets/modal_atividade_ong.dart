...
Future<void> abrirModalAtividadeOng({
  ...
  final nomeCtrl = TextEditingController(text: atividade?['nomeServico']?.toString() ?? '');
  final descCtrl = TextEditingController(text: atividade?['descricao']?.toString() ?? '');
  final duracaoCtrl = TextEditingController(text: atividade?['horas']?.toString() ?? '');
          ...
          Future<void> confirmar() async {
            ...
            if (resultado['sucesso'] == true) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text(editando ? 'Atividade atualizada com sucesso!' : 'Atividade cadastrada com sucesso!')),
              );
              onSucesso();
            } else {
              ScaffoldMessenger.of(modalContext).showSnackBar(
                SnackBar(
                  content: Text(
                    resultado['erro']?.toString() ??
                        (editando ? 'Não foi possível atualizar a atividade.' : 'Não foi possível cadastrar a atividade.'),
                  ),
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Container(
                          width: 44,
                          height: 44,
                          decoration: const BoxDecoration(shape: BoxShape.circle, color: Color(0xFFE5E2D8)),
                          child: ClipOval(child: Image.asset('assets/images/logo.png')),
                        ),
                        const SizedBox(width: 12),
                        Expanded(
                          child: Text(
                            editando ? 'Editar atividade' : 'Cadastro de atividade',
                    ...
                    _modalInput(controller: nomeCtrl, hint: 'Ex: Aula de reforço escolar'),
                    ...
                    Container(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: 'Descreva a atividade, público-alvo e objetivos...',
                          hintStyle: GoogleFonts.quicksand(fontSize: 12, color: const Color(0xFF9E9E9E)),
                          filled: true,
                          fillColor: const Color(0xFFE5E2D8),
                          counterStyle: GoogleFonts.quicksand(fontSize: 11, color: Colors.white70),
                      ...
                      children: [
                              ...
                              Container(
                                ...
                                child: DropdownButtonHideUnderline(
                                  child: DropdownButton<String>(
                                    ...
                                    isExpanded: true,
                                    hint: Text('Selecionar', style: GoogleFonts.quicksand(fontSize: 13, color: const Color(0xFF6B705C))),
                                    items: focosDisponiveis
                                        .map((f) => DropdownMenuItem(
                                              value: f,
                                              child: Text(f, style: GoogleFonts.quicksand(fontSize: 13, color: const Color(0xFF344E41))),
                                            ))
                                        .toList(),
                        ...
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              _modalLabel('Duração', Icons.access_time_rounded),
                              _modalInput(controller: duracaoCtrl, hint: 'Ex: 2 horas'),
                            ],
                          ),
                        ),
                      ],
                    ...
                    Row(
                      children: [
                            ...
                            child: Container(
                              ...
                              child: Center(
                                child: Column(
                                  ...
                                  children: [
                                    const Icon(Icons.file_upload_outlined, color: Colors.white70, size: 26),
                                    const SizedBox(height: 6),
                                    Text(
                                      editando && imagemSelecionada == null ? 'Trocar imagem' : 'Clique para enviar',
                          ...
                          Stack(
                            children: [
                              ClipRRect(
                                ...
                                child: kIsWeb
                                    ? Image.network(imagemSelecionada!.path, width: 76, height: 76, fit: BoxFit.cover)
                                    : Image.file(File(imagemSelecionada!.path), width: 76, height: 76, fit: BoxFit.cover),
                              ),
                              Positioned(
                                bottom: 2,
                                right: 2,
                        ...
                        ] else if (editando && (atividade!['imagem']?.toString().isNotEmpty ?? false)) ...[
                          const SizedBox(width: 12),
                          Stack(
                            children: [
                              ClipRRect(
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: Text(
                          enviando ? 'Enviando...' : (editando ? 'Salvar alterações' : 'Cadastrar atividade'),
                    ...
                    Center(
                      child: TextButton(
                        onPressed: enviando ? null : () => Navigator.pop(modalContext),
                        child: Text('Cancelar', style: GoogleFonts.quicksand(color: Colors.white70)),
                      ),
                    ),
                  ],
                ),
...
Widget _modalLabel(String texto, IconData icone) {
  return Row(
    children: [
      ...
      Text(
        texto,
        style: GoogleFonts.quicksand(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.w700),
      ),
    ],
  );
}

Widget _modalInput({required TextEditingController controller, required String hint}) {
  return Container(
    margin: const EdgeInsets.only(top: 5),
    child: TextField(
