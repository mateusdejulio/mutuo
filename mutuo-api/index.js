...
app.post('/loginUsuario', async (req, res) => {
  ...
  try {
    ...
    if (resultado.sucesso) {
      return res.json({ sucesso: true, usuario: resultado.usuario });
    } else {
      return res.status(401).json({ sucesso: false, mensagem: resultado.mensagem || 'Email ou senha incorretos' });
    }
...
// Rota de login para ONGs — autentica na tabela Mutuo_ONG
